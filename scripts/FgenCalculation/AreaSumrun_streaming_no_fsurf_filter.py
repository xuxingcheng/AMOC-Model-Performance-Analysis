...
DEFAULT_OUTPUT = os.path.join(
    fgen.DATA_ROOT,
    "AreaSum_Allmodels_no_fsurf_filter_streaming.pkl",
)
...
def parse_args():
    parser = argparse.ArgumentParser(
        description=(
            ...
            "grid cells where fsurf is finite, rho exceeds the threshold, and "
            "the existing North Atlantic spatial mask is true. No fsurf sign "
            "filter is applied."
        )
    )
...
def compute_areasum_for_model(model, fsurf_ds, area_da, rho_min):
    ...
    areasum.attrs["fsurf_sign_filter"] = "none"
    areasum.attrs["description"] = (
        "AreaSum is the sum of areacello where fsurf is finite, rho > rho_min, "
        "and the existing North Atlantic mask is true. No fsurf sign filter is "
        "applied."
    )
    return areasum
